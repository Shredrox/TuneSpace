...
internal class LastFmClient(HttpClient httpClient) : ILastFmClient
{
    const string ApiKey = "";
    const string BaseUrl = "https://ws.audioscrobbler.com/2.0/";

    async Task<BandModel> ILastFmClient.GetBandDataAsync(string bandName)
    {
        var apiUrl = $"{BaseUrl}?method=artist.getInfo&artist={bandName}&api_key={ApiKey}&format=json";
    ...
    {
        var apiUrl = $"{BaseUrl}?method=artist.getSimilar&artist={bandName}&api_key={ApiKey}&limit={limit}&format=json";
        var response = await httpClient.GetStringAsync(apiUrl);
